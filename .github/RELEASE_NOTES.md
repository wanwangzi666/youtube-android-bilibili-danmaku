...
- **Shorts（竖屏短视频）可跳过**：自动判断当前是否在 Shorts 播放器里，关掉「Shorts 也匹配弹幕」后
  进入 Shorts 不再搜索、不显示弹幕与悬浮按钮（**本版本新增**）

## 本版本更新（1.1.0）

**新增：Shorts 识别与跳过**

刷 Shorts 时，模块此前会按短视频标题去 B 站搜索，结果匹配度只有 0~25%，然后弹出
...
- **模块设置**新增「Shorts（竖屏短视频）里也匹配并显示弹幕」（默认开，与旧版行为一致）
- **播放页悬浮面板**里也能改，会话内即时生效，并在状态区显示 Shorts 判定依据（视图 / 几何），
  方便反馈误判
...
1. 下载本页附件里的 `B2Y-Vector-*.apk`，安装：
   ```bash
   adb install -r B2Y-Vector-1.1.0.apk
   ```
...
- 不想在 Shorts 里看到弹幕：模块设置或播放页悬浮面板里关掉「Shorts 也匹配弹幕」
...
- 构建与测试：**编译通过，118 个 JVM 单元测试全部通过**（本 Release 由 CI 自动构建并跑完测试后产出）

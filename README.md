...
- 🐛 修「设置里取消了勾选，YouTube 里仍然匹配 Shorts」。诊断信息显示
  `已识别为 Shorts，但设置=允许匹配` —— 识别是好的，坏的是**设置读取**：
  模块 App 保存的值没能通过 `XSharedPreferences` 同步到被注入的 YouTube 进程。
  修法是给这个开关加一条**不依赖跨进程机制**的通道：
  * 新增**运行时开关**（被注入进程自己写自己的 SharedPreferences，`b2y_runtime`），
    生效优先级为 **运行时开关 > 模块 App 的 JSON 配置**
  * **播放页面板顶部新增全局开关**「在 Shorts 里也匹配弹幕」：立即生效、立即保存，
    不用重启 YouTube（也绕开了设置页那条路径）
...
- 🐛 **改用「底部导航的 Shorts 标签是否选中」判断是否在 Shorts 里**（用户建议的判定方式）。
  之前的两种依据在部分机型/版本上都没命中。现在三层判定：
  **Shorts 标签选中 + 竖屏画面** / Shorts 播放器视图 / 竖屏铺满几何。
  标签判定只用框架 API（`findViewsWithText` + `selected` 状态）遍历视图树，不 hook 任何
  YouTube 内部类，且要求标签位于屏幕下半部分，避免把标题里的 `#shorts` 当成导航标签
- 🐛 自动匹配的闸门加到 4 处：新增「弹幕下载完成、准备贴到画面前」的检查
- 💄 设置页把该项拆成独立分组，文案改为「在 Shorts 里也匹配并显示弹幕」

### 1.1.1

修复与变更：

- 💄 「Shorts 里也匹配弹幕」默认值改为「不匹配」：刷短视频本来就不该有弹幕。
  全新安装默认不匹配；配置里没有这个键的旧版本升级时会按新默认值读取；
  显式设置过的用户仍以自己的选择为准
- 🐛 修 1.1.0 的 Shorts 屏蔽没真正生效的问题：1.1.0 只在「发起搜索」那一刻判断，而搜索是
  异步的 —— 结果回来时用户可能已经滑进 Shorts，「选择要同步的 B 站视频」照样弹出来。
  闸门加到 3 处：**发起自动匹配时 / 准备显示选择弹窗时 / 浮层每轮刷新时**（后者会把已经
  弹出来但此时已在 Shorts 的弹窗自动关掉并清空弹幕）
- 🐛 判定不再依赖时序：浮层每 250ms 才喂一次画面矩形，在此之前判定恒为「不是 Shorts」；
...
- ✨ 新增 **Shorts（竖屏短视频）识别与跳过**：自动判断当前是否在 Shorts 播放器里
  （Shorts 播放器类名 + 竖屏铺满几何两层判定），可在模块设置或播放页面板里切换
  「Shorts 也匹配弹幕」

...
2. 右上角悬浮 **「弹」** 按钮（可拖动）打开控制面板：
   ...
   | 分组 | 内容 |
   | --- | --- |
   | **全局设置** | 「匹配并显示弹幕（总开关）」+「在 Shorts 里也匹配」两个**全局开关**，点一下立即生效并保存 |
   | **识别与加载** | 重新搜索 / 搜索关键词 / 粘贴 B 站链接 / 番剧模式 |
   | **时间轴** | ±0.5s 微调、归零、毫秒级滑杆 |
   | **显示** | 不透明度 / 字号 / 速度 / 显示区域 / 权重过滤 / 清空弹幕 |
   | **诊断** | 当前 `videoId`、标题、匹配到的 `bvid`、弹幕条数、播放位置、两个开关的状态、Shorts 判定依据、渲染帧率，可一键复制 |

   **总开关**（面板顶部「全局设置」）关掉后：不再自动匹配，弹幕层隐藏；
   悬浮「弹」按钮会保留，方便你随时开回来。手动加载（粘贴链接 / 搜关键词）不受影响。

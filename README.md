...
- 🐛 **改用「底部导航的 Shorts 标签是否选中」判断是否在 Shorts 里**（用户建议的判定方式）。
  之前的两种依据在部分机型/版本上都没命中，导致勾选与取消勾选看起来「都没效果」。
  现在三层判定：**Shorts 标签选中 + 竖屏画面** / Shorts 播放器视图 / 竖屏铺满几何。
  标签判定只用框架 API（`findViewsWithText` + `selected` 状态）遍历视图树，不 hook 任何
  YouTube 内部类，且要求标签位于屏幕下半部分，避免把标题里的 `#shorts` 当成导航标签
- 🐛 自动匹配的闸门加到 4 处：新增「弹幕下载完成、准备贴到画面前」的检查，下载期间滑进
  Shorts 会直接丢弃结果
- 💄 播放页面板里不再重复放这个开关（同一设置出现在两处容易互相打架），统一由模块设置页
  「Shorts（竖屏短视频）」一节控制；面板诊断区会明确区分**「已识别为 Shorts」**和
  **「识别到了但设置允许匹配」**，便于判断是判定问题还是设置问题
- 💄 设置页把该项拆成独立分组，文案改为「在 Shorts 里也匹配并显示弹幕」，去掉了原来
  又长又绕的说明
